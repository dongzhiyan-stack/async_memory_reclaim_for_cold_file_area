...
线上实际测试证实，有不少文件的pagecache在100M~1G之间，但是systemtap动态跟踪并打印这些文件被读写文件页page索引。发现总是只打印几个固定的文件页page索引，这就说明这些文件的大部分文件页都基本不被访问。如果能准确识别出这些冷page并回收掉，将腾出大量内存，还基本不会发生refault，内存回收效率会很高！
