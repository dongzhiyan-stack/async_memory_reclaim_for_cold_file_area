...
struct hot_cold_file_shrink_counter
{
	...
	unsigned int file_area_delete_in_cache_count;//释放的file_area但是处于hot_file_area_cache数组的file_area个数
	unsigned int file_area_cache_hit_count;//从hot_file_area_cache命中file_area次数

	unsigned int file_area_access_count_in_free_page;//file_area内存回收期间file_area被访问的次数
	unsigned int hot_file_area_count_in_free_page;//在内存回收期间产生的热file_area个数
	unsigned int refault_file_area_count_in_free_page;//在内存回收期间产生的refault file_area个数

	unsigned int hot_file_area_count_one_period;//一个周期内产生的热file_area个数
	unsigned int refault_file_area_count_one_period;//一个周期内产生的refault file_area个数
};
#define FILE_AREA_CACHE_COUNT 5//一个file_stat结构里缓存的热file_area结构个数
#define ASYNC_MEMORY_RECLAIM_ENABLE 0//置1才允许异步内存回收
#define ASYNC_DROP_CACHES 1//置1说明说明触发了drop_cache，此时禁止异步内存回收线程处理gloabl drop_cache_file_stat_head链表上的file_stat
...
struct hot_cold_file_global
{
	...
	struct list_head drop_cache_file_stat_head;//触发drop_cache后的没有file_stat的文件，分配file_stat后保存在这个链表
    unsigned int drop_cache_file_count;//触发drop_cache后的没有file_stat的文件个数
};
...
//FILE_AREA_LIST_STATUS(cold_list)
...
#define CLEAR_FILE_AREA_STATUS(status) \
	static inline void clear_file_area_in_##status(struct file_area *p_file_area)\
    {clear_bit(F_file_area_in_##status,&p_file_area->file_area_state);}
//设置file_area的状态
#define SET_FILE_AREA_STATUS(status) \
	static inline void set_file_area_in_##status(struct file_area *p_file_area)\
    {set_bit(F_file_area_in_##status,&p_file_area->file_area_state);}
//测试file_area的状态
#define TEST_FILE_AREA_STATUS(status) \
	static inline int file_area_in_##status(struct file_area *p_file_area)\
    {return test_bit(F_file_area_in_##status,&p_file_area->file_area_state);}
 ...
 * 并且此时要避免此时有进程在执行hot_file_update_file_status()函数。这些在hot_file_update_file_status()和get_file_area_from_file_stat_list()函数有说明
 * 其实file_stat设置in_free_page、free_page_done 状态都有spin lock加锁，不使用set_bit、clear_bit也行，目前暂定先用set_bit、clear_bit吧，后续再考虑其他优化*/
		...
		struct list_head *file_area_free)
{
	...
	list_for_each_entry_safe(p_file_area,tmp_file_area,file_area_free,file_area_list){
		...
		if(test_bit(F_file_stat_lock_not_block,&p_file_stat->file_stat_status))//if成立说明有inode删除lock_file_stat()获取锁失败。这里立即结束遍历file_stat的file_area，因为inode都要删除了
		{
...
static int drop_cache_truncate_inode_pages(struct hot_cold_file_global *p_hot_cold_file_global)
{
    ...
    list_for_each_entry_safe_reverse(p_file_stat,p_file_stat_temp,&p_hot_cold_file_global->drop_cache_file_stat_head,hot_cold_file_list){
        ...
        if(0 == p_file_stat->file_area_count){
			...
			//file_stat加锁，防止此时inode并发被删除了。如果删除了则p_file_stat->mapping 是NULL，不再处理。此时文件file_stat也会自动被加上delete标记，然后删除掉file_stat。
			...
			//到这里，是否可以通过对inode引用计数加1防止inode被删除呢？这样就避免长时间使用lock_file_stat锁，因为file_stat_truncate_inode_pages()过程可能比较耗时！不行，分析见file_stat_free_leak_page函数
            ...
            /*把文件file_stat移动到file_stat_zero_file_area_head链表，如果一段时间改文件还是没被访问，则释放掉file_stat.
			  因为此时hot_file_update_file_status()中可能并发把file_stat移动到global 热文件或者大文件链表，因此需要global_lock加锁。错了，不用加锁，因为这些file_stat
			  目前还没有状态，不是in temp list状态。而hot_file_update_file_status()中只有file_stat处于in temp list状态，才会移动到global 热文件和大文件链表*/
            ...
            //把file_stat移动到global file_stat_zero_file_area_head链表尾巴，这样file_stat可以优先被回收掉。并且回收前还会再被强制回收一次pagecache，有点浪费性能。
			//是否就在这里把或则file_stat释放掉得了???????????????????????????
		...
		{
		/*到这个分支，说明drop_caches的文件被访问了，分配file_area，那就不管它了。让这个文件file_stat按照正常的异步内存回收参与回收page，等到file_area对应的page被释放完后。
		  文件file_stat一个file_area都没有，然后被移动到p_hot_cold_file_global->file_stat_zero_file_area_head链表。而如果p_hot_cold_file_global->file_stat_zero_file_area_head，
		  链表上的file_stat还有很多pagecache，说明这些pagecache没有被file_area统计到，于是就会执行file_stat_truncate_inode_pages()强制释放掉这部分pagecache。因此，这个else分支
		  的文件file_stat的所有pagecache也能被完美释放掉，就是时间比较长而已。因此，这里只用把file_stat移动到global file_stat_temp_head链表，让这个文件file_stat参与正常的异步
		  内存回收。但是这个过程需要加锁，因为file_stat设置了in temp list状态
		  */
			spin_lock(&p_hot_cold_file_global->global_lock);
			clear_file_stat_in_drop_cache(p_file_stat);
...
//global file_stat_zero_file_area_head链表上的file_stat，一个file_area都没有，但是还有pagecache，因为这些pagecache没有访问，被文件file_stat的file_area统计到。这个函数则强制释放掉这些文件的pagecache。
void file_stat_free_leak_page(struct hot_cold_file_global *p_hot_cold_file_global,struct file_stat *p_file_stat)
{
    ...
    /*到这里，说明文件inode没有被释放，我觉得可以模仿drop_pagecache_sb()，先spin_lock(&inode->i_lock)对inode加锁，然后执行__iget(inode)令inode引用计数加1.然后执行invalidate_mapping_pages()
	 * 放心截断释放文件的pagecache。最后用完inode再令inode引用计数减1，然后inode才可以被释放掉。不行，有个并发问题，到这里时，文件inode可能正好引用计数减1变为0，然后去释放文件inode。
	 * 我在这里对inode引用计数加1就失效了。但是全程lock_file_stat()加锁防护，可以保证inode删除执行到__destroy_inode_handler_post()时，获取file_stat锁失败而阻塞，暂定释放inode结构，这样才没事*/

	/*如果文件mapping统计到的pagecache数大于file_stat统计到的pagecache数，则强制截断释放文件pagecache。这里会有统计误差，因为默认一个文件file_area对应4个page。
	  但是一个file_area实际可能只包含了1个page，因为只有一个page被访问了。不过强制截断释放文件pagecache，也没啥事。p_file_stat->file_area_count基本都是0，
	  但是可能正好此时文件被访问了，p_file_stat->file_area_count就大于0了*/
...
static inline int  add_file_to_file_stat(struct address_space *mapping)
{
    ...
    if(mapping->rh_reserved1 == 0 && mapping->nrpages > 5){
		...
		/*这里有个问题，如果此时该文件被访问了，执行了hot_file_update_file_status()，也会为该文件分配file_stat并赋值给mapping->rh_reserved1，这样就有问题了!!!!!!!!!!!!!
		 * 一个文件分配两个file_stat，泄漏了。需要防止这种并发，用global_lock spin lock锁最简单!!!!!!!!!!!!*/
		spin_lock(&hot_cold_file_global_info.global_lock);
		//极端情况是，先有进程在hot_file_update_file_status()获取global lock锁，分配file_stat并赋值给mapping->rh_reserved1。然后该函数获取glabal lock锁，就直接return返回了。
...
int hot_file_update_file_status(struct page *page)
{
	...
	if(mapping && mapping->host && mapping->host->i_sb/* && (async_memory_reclaim_status == mapping->host->i_sb->s_dev || async_memory_reclaim_status == mapping->host->i_sb->s_dev >> 20)*/)
	{
		...
		//还要再判断一次async_memory_reclaim_status是否是0，因为驱动卸载会先获取原子变量ref_count的值0，然后这里再执行atomic_inc(&hot_cold_file_global_info.ref_count)令ref_count加1.
		//这种情况必须判断async_memory_reclaim_status是0，直接return返回。否则驱动卸载过程会释放掉file_stat结构，然后该函数再使用这个file_stat结构，触发crash
		if(!test_bit(ASYNC_MEMORY_RECLAIM_ENABLE,&async_memory_reclaim_status))
			goto err;
...
already_alloc:	    
		...
		if(p_file_area->access_count == 1)
		{
			...
			if(file_area_in_free_list(p_file_area)){//file_area是in_free_list状态
...
repeat:
	...
	/*到这里还残留在global_file_stat_temp_head_list上的file_stat，是本轮就没有扫描到的。因为参与内存回收的扫描的file_area总数不能超过scan_file_area_max，
	 * 如果某个file_stat的file_area太多就会导致global_file_stat_temp_head_list链表上其他file_stat扫描不到。这里要把这些file_stat移动到
	 * global file_stat_temp_head链表尾，下次异步内存回收继续扫描这些file_stat*/
...
unsigned long free_page_from_file_area(struct hot_cold_file_global *p_hot_cold_file_global,struct list_head * file_stat_free_list,struct list_head *file_stat_temp_head)
{
	...
	//需要调度的话休眠一下
	cond_resched();
	//在内存回收结束时，遍历参与内存回收的一个个文件file_stat的file_area_temp和file_area_free链表头的file_area，是否在内存回收期间被访问了，是的话就移动到对应链表
    list_for_each_entry(p_file_stat,file_stat_free_list,hot_cold_file_list){
		//如果内存回收期间file_stat->file_area_temp链表上的file_area被频繁访问，这种file_area只会移动到file_stat->file_area_temp链表头。
		//这里在内存回收结束时，检查file_stat->file_area_temp链表头是否有热file_area，有的话就释放则移动到file_stat->file_area_hot链表，没有立即跳出循环
		list_for_each_entry_safe(p_file_area,p_file_area_temp,&p_file_stat->file_area_temp,file_area_list){
			if(!file_area_in_temp_list(p_file_area) || file_area_in_temp_list_error(p_file_area))
				panic("%s file_area:0x%llx status:%d not in file_area_temp\n",__func__,(u64)p_file_area,p_file_area->file_area_state);

			if(p_file_area->access_count > FILE_AREA_HOT_LEVEL){
				spin_lock(&p_file_stat->file_stat_lock);
				clear_file_area_in_temp_list(p_file_area);
				set_file_area_in_hot_list(p_file_area);
		...
		//这里在内存回收结束时，检查file_stat->file_area_free链表头的file_area是否内存回收过程或结束时被访问了，是则释放则移动到file_stat->file_area_refault链表，无则立即跳出循环
		list_for_each_entry_safe(p_file_area,p_file_area_temp,&p_file_stat->file_area_free,file_area_list){
			if(!file_area_in_free_list(p_file_area) || file_area_in_free_list_error(p_file_area))
				panic("%s file_area:0x%llx status:%d not in file_area_free\n",__func__,(u64)p_file_area,p_file_area->file_area_state);

			if(p_file_area->access_count > 0){
				spin_lock(&p_file_stat->file_stat_lock);
				clear_file_area_in_free_list(p_file_area);
				set_file_area_in_refault_list(p_file_area);
				list_move(&p_file_area->file_area_list,&p_file_stat->file_area_refault);
	...
	if(!list_empty(file_stat_free_list)){
		spin_lock(&p_hot_cold_file_global->global_lock);
		/*突然想到，下边for循环正在进行时，要是hot_file_update_file_status()函数中把p_file_stat的下一个file_stat即p_file_stat_temp移动到其他链表了
		 * 这个for循环岂不是又要发生file_stat跨链表导致死循环?不会，因为这个for循环全程 global_lock加锁，其他地方无法把file_stat移动到其他链表*/
...
static void printk_shrink_param(struct hot_cold_file_global *p_hot_cold_file_global,struct seq_file *m,int is_proc_print)
{
	...
	if(is_proc_print){
		...
		seq_printf(m,"file_area_delete_in_cache:%d file_area_cache_hit:%d file_area_access_in_free_page:%d hot_file_area_in_free_page:%d refault_file_area_in_free_page:%d hot_file_area_one_period:%d refault_file_area_one_period:%d\n",p->file_area_delete_in_cache_count,p->file_area_cache_hit_count,p->file_area_access_count_in_free_page,p->hot_file_area_count_in_free_page,p->refault_file_area_count_in_free_page,p->hot_file_area_count_one_period,p->refault_file_area_count_one_period);
	...
	else
	{
		...
		printk("file_area_delete_in_cache_count:%d file_area_cache_hit_count:%d file_area_access_count_in_free_page:%d hot_file_area_count_in_free_page:%d refault_file_area_count_in_free_page:%d hot_file_area_count_one_period:%d refault_file_area_count_one_period:%d\n",p->file_area_delete_in_cache_count,p->file_area_cache_hit_count,p->file_area_access_count_in_free_page,p->hot_file_area_count_in_free_page,p->refault_file_area_count_in_free_page,p->hot_file_area_count_one_period,p->refault_file_area_count_one_period);
 ...
 * */
void file_stat_has_zero_file_area_manage(struct hot_cold_file_global *p_hot_cold_file_global)
{
	struct file_stat * p_file_stat,*p_file_stat_temp;
...
//卸载该驱动时，先async_memory_reclaim_status=0，确保所有的file_stat和file_area不再被进程访问后。就会执行该函数删除掉所有文件对应的file_stat，同时要把file_stat->mapping->rh_reserved1清0，
...
void cold_file_disable_file_stat_mapping(struct hot_cold_file_global *p_hot_cold_file_global,struct file_stat * p_file_stat)
{
	 ...
	 * cold_file_disable_file_stat_mapping()的执行时机是卸载驱动，卸载驱动时先async_memory_reclaim_status = 0,然后smp_wmb()。然后__destroy_inode_handler_post()函数中，先smp_rmb()获取最新的
	 * async_memory_reclaim_status值，如果async_memory_reclaim_status=0，则不再执行set_file_stat_in_delete(p_file_stat)。但是把inode释放后，再执行cold_file_disable_file_stat_mapping()
...
//#endif		
			...
			{   /*到这个分支说明async_memory_reclaim_status已经被驱动卸载并发清0了，那就goto file_stat_delete分支，择机把inode->i_mapping->rh_reserved1清0，保证这个inode
				  被新的进程读写文件分配后，因文件访问执行hot_file_update_file_status()时，inode->i_mapping->rh_reserved1是0，则重新分配一个新的file_stat，否则会使用
				  inode->i_mapping->rh_reserved1指向的老的已经释放的file_stat*/
...
{ 
	//clear_bit(0, &async_memory_reclaim_status);//驱动卸载，把async_memory_reclaim_status清0
	
	//这里是重点，先等异步内存回收线程结束运行，就不会再使用任何的file_stat了，此时可以放心执行下边的cold_file_delete_all_file_stat()释放所有文件的file_stat
	kthread_stop(hot_cold_file_global_info.hot_cold_file_thead);

	//async_memory_reclaim_status = 0;------改为使用 clear_bit()把async_memory_reclaim_status清0，这样使用async_memory_reclaim_status的地方不用再smp_rmb获取最新的async_memory_reclaim_status值0
	//smp_wmb();
